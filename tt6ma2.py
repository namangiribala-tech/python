...
b=set(students["branches"])

#create a dictionary to display the topper inforation 
report={
    "total students": len(students),
    "branches":set(i["branches"] for i in students),
    "toppers":{
        {"overall":max(i["percentage"] for i in students)},
        "branch toppers": {max(filter(lambda x:x["branch"]==b, students), key=lambda x:x["percentage"]) for b in set(i["branch"] for i in students)}
        }
    }
